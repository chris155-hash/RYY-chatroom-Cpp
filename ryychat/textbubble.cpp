...
{
    m_pTextEdit = new QTextEdit();//用Qt提供的QTextEdit。
...
{
    if (m_pTextEdit == o && e->type() == QEvent::Paint){ //如果是文本pTextEdit触发事件，且事件类型是喷绘则调整文本高度。比如拉伸窗口，气泡大小调整
        adjustTextHeight();//调整文本高度//PaintEvent中设置
    }
...
{
    m_pTextEdit->setStyleSheet("QTextEdit{background:transparent;border:none}");//TextEdit设置无边框
}
