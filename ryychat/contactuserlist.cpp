...
void ContactUserList::addContactUserList()
{
    ...
    //1.向QListWidget里添加我们自定义的Item的逻辑，先创立我们要添加的_add_friend_item，2.再创建QListWidgetItem *add_item;
    //3.把第二步创建的*add_item加到ContactUserList里  4.最后this->setItemWidget(add_item, _add_friend_item);用我们的_add_friend_item替换第三部添加的add_item
...
{
    QWidget *widget = this->itemWidget(item);//把item对应的自定义 widget 取出来。获取自定义widget对象
    if(!widget){
    ...
    ListItemBase *customItem = qobject_cast<ListItemBase*>(widget);//子类转基类，完全ok
    if(!customItem){
        qDebug()<< "slot item clicked widget is nullptr";
        return;
    }
