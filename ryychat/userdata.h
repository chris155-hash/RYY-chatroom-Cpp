...
class TextChatData;//前置声明一下，实际定义在别处。只是这里下面要用，相当于先空定义一下
struct FriendInfo{
...
struct UserInfo {
    ...
    int _sex;
    QString _last_msg;
    std::vector<std::shared_ptr<TextChatData>> _chat_msgs;//其他用户与自己的聊天信息记录
};

struct TextChatData{   //这只是一条聊天消息，发起来太麻烦了。所以要组装成TextChatMsg
