...
bool RedisMgr::Auth(const std::string& password)
{
	...
	if (_reply->type == REDIS_REPLY_ERROR) {
		...
		freeReplyObject(_reply);  //执行成功 释放redisCommand执行后返回的redisReply所占用的内存
		return false;
	}
	else {
		freeReplyObject(_reply);  //执行成功 释放redisCommand执行后返回的redisReply所占用的内存
...
RedisConPool::~RedisConPool()
{
	...
	while (!connections_.empty()) {
		redisFree(connections_.front());//ryy补充
		connections_.pop();
	}
}
...
redisContext* RedisConPool::getConnection() {
	...
	cond_.wait(lock, [this] {    //解锁的两个条件：1.连接池要关闭返回空指针 2.有空闲的连接了返回该连接的上下文
		if (b_stop_ == true) {
			return true;
		}
...
void RedisConPool::returnConnection(redisContext* context) //向连接池里归还连接
{
	std::lock_guard<std::mutex> lock(mutex_);
	if (b_stop_) return;
	connections_.push(context);
